...
class UTCPAgentTool(AgentTool):
    ...
    def tool_spec(self) -> ToolSpec:
        ...
        if self.utcp_tool.inputs.title:
            input_schema["title"] = self.utcp_tool.inputs.title
        if self.utcp_tool.inputs.items:
            input_schema["items"] = self.utcp_tool.inputs.items
        if self.utcp_tool.inputs.enum:
            input_schema["enum"] = self.utcp_tool.inputs.enum
        if self.utcp_tool.inputs.minimum is not None:
            input_schema["minimum"] = self.utcp_tool.inputs.minimum
        if self.utcp_tool.inputs.maximum is not None:
            input_schema["maximum"] = self.utcp_tool.inputs.maximum
