...
def mock_utcp_tool():
    ...
    provider = HttpProvider(name="weather_provider", url="https://api.weather.com/utcp")
...
def test_tool_spec_with_optional_fields(mock_utcp_client):
    ...
    provider = HttpProvider(name="test_provider", url="http://example.com")
    tool = UTCPTool(
        name="test_tool", description="Test tool with optional fields", inputs=input_schema, tool_provider=provider
    )
    ...
    assert input_json["enum"] == ["option1", "option2"]
    assert input_json["items"] == {"type": "string"}
...
async def test_stream(utcp_agent_tool, mock_utcp_client, alist):
    ...
    exp_events = [mock_utcp_client.call_tool_async.return_value]
