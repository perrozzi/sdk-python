...
def sample_utcp_tools():
    ...
    provider1 = HttpProvider(name="weather_api", url="https://weather.com/utcp")
    provider2 = HttpProvider(name="news_api", url="https://news.com/utcp")

    input_schema1 = ToolInputOutputSchema(
        type="object", properties={"location": {"type": "string"}}, required=["location"]
    )

    input_schema2 = ToolInputOutputSchema(
...
async def test_utcp_client_context_manager(utcp_config, mock_utcp_native_client):
    ...
    with patch("strands.tools.utcp.utcp_client.UtcpClient.create", new_callable=AsyncMock) as mock_create:
        ...
        async with UTCPClient(utcp_config) as client:
            ...
            mock_create.assert_called_once_with(config=utcp_config)
...
def test_list_tools_sync_success(utcp_client_sync, sample_utcp_tools):
    ...
    utcp_client_sync._utcp_client.tool_repository.get_tools.return_value = sample_utcp_tools
...
async def test_call_tool_async_success_dict_result(utcp_client):
    ...
    utcp_client._utcp_client.call_tool.assert_called_once_with(
        tool_name="weather.get_current", arguments={"location": "London"}
    )
...
def test_call_tool_sync_success(utcp_client_sync):
    ...
    # Mock asyncio.run to avoid actually running async code in sync test
    with patch("asyncio.run") as mock_run:
        mock_run.return_value = mock_result
        utcp_client_sync._utcp_client.call_tool.return_value = mock_result
